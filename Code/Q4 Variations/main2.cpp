...
const int RED_THRESHOLD   = 230; //Value for which the component of a pixel will be considered red.   #todo take picture and check best values.
const int GREEN_THRESHOLD = 200; //Value for which the component of a pixel will be considered green. #todo take picture and check best values.
const int BLUE_THRESHOLD  = 200; //Value for which the component of a pixel will be considered blue.  #todo take picture and check best values.
const int MIN_RED_COUNTER = 60;  //Number of reddish pixels a line must have to be considered red.
...
bool leftWall(){
    bool is_close        = false;
    int  digital_reading = read_analog(L_SENSOR);
    if (digital_reading < 500)
        is_close = true;
    return is_close;
}
...
bool rightWall(){
    bool is_close        = false;
    int  digital_reading = read_analog(R_SENSOR);
    if (digital_reading < 500)
        is_close = true;
    return is_close;
}
...
void q4Control(double current_state){
    ...
    double derivative            = 0;
    double duty_cycle_correction = error_percentage*KPQ4 + derivative*KDQ4;
...
int main(){
    ...
    while(quad == 3){
        ...
        if (front_reading>MIN_DISTANCE){
            ...
            if(h_data.total_white_pixels >= TRANSVERSAL){
                ...
                h_data = getHorizontalData(ROW-30);
                while(h_data.white_pixels1 < MIN_H_TRACK_WID){
    ...
    while(quad == 4){
        ...
        front_reading = readAnalogSensor(F_SENSOR, 5).average;
        if (front_reading < MIN_DISTANCE){
            ...
            //The MIN_DISTANCE here corresponds to the minimum distance that allows the robot to
            //safely make the turns, and the MIN_DISTANCE-30 is value that guarantees the robot
            //is not seeing walls ahead of it, i.e., it completed the turn.
            
            if (leftWall() && !rightWall()){
                //Turns to the right until front sensor stops detecting walls.
                while (front_reading > MIN_DISTANCE-30){ //#todo find a good value for this bigger distance
                    ...
                    if (front_reading < MIN_DISTANCE){
                        ...
                        //difference in the distance? i.e., proportional to (current reading - MIN_DISTANCE)
                    }
    ...
    while(quad == -1){
        ...
        while(true){
            front_reading = readAnalogSensor(F_SENSOR, 1).average;
            int left_reading  = readAnalogSensor(L_SENSOR, 1).average;
            int right_reading = readAnalogSensor(R_SENSOR, 1).average;
